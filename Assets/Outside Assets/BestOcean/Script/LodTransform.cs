...
public class LodTransform : MonoBehaviour
{
    ...
    {
        float oceanBaseScale = Ocean.Instance.transform.lossyScale.x;
        float maxDiameter = 4f * oceanBaseScale * Mathf.Pow(2f, _lodIndex);
        float maxTexelSize = maxDiameter / (4f * Ocean.Instance._baseVertDensity);
        return 2f * maxTexelSize * Ocean.Instance._minTexelsPerWave;
    }
