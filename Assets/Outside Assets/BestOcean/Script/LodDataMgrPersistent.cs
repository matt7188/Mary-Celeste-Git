...
public abstract class LodDataMgrPersistent : LodDataMgr
{
    ...
    protected override void InitData()
    {
        ...
        _sources = new RenderTexture[Ocean.Instance.CurrentLodCount];
    ...
    public void BindSourceData(int lodIdx, int shapeSlot, Material properties, bool paramsOnly, bool usePrevTransform)
    {
        ...
        var rd = usePrevTransform ?
            Ocean.Instance._lods[lodIdx]._renderDataPrevFrame.Validate(BuildCommandBufferBase._lastUpdateFrame - Time.frameCount, this)
            : Ocean.Instance._lods[lodIdx]._renderData.Validate(0, this);
    ...
    public override void BuildCommandBuffer(Ocean ocean, CommandBuffer buf)
    {
        ...
        for (int stepi = 0; stepi < steps; stepi++)
        {
            ...
            for (var lodIdx = lodCount - 1; lodIdx >= 0; lodIdx--)
            {
                ...
                _renderSimMaterial[stepi, lodIdx].SetFloat("_GridSize", Ocean.Instance._lods[lodIdx]._renderData._texelWidth);
