...
public class LodDataMgrAnimWaves : LodDataMgr
{
    ...
    protected override void InitData()
    {
        ...
        _combineMaterial = new Material[Ocean.Instance.CurrentLodCount];
        for (int i = 0; i < _combineMaterial.Length; i++)
        {
            _combineMaterial[i] = new Material(Shader.Find("Hidden/Ocean/Combine Animated Wave LODs"));
        }

        int resolution = Ocean.Instance.LodDataResolution;
        var desc = new RenderTextureDescriptor(resolution, resolution, TextureFormat, 0);

        _waveBuffers = new RenderTexture[Ocean.Instance.CurrentLodCount];
    ...
    public override void BuildCommandBuffer(Ocean ocean, CommandBuffer buf)
    {
        base.BuildCommandBuffer(ocean, buf);

        var lodCount = Ocean.Instance.CurrentLodCount;
        ...
        for (int lodIdx = lodCount - 1; lodIdx >= 0; lodIdx--)
        {
            ...
            var lodMaxWavelength = Ocean.Instance._lods[lodIdx].MaxWavelength();
    ...
    public void BindWaveBuffer(int lodIdx, int shapeSlot, Material properties, bool paramsOnly)
    {
        ...
        var rd = Ocean.Instance._lods[lodIdx]._renderData.Validate(0, this);
    ...
    public static int SuggestDataLOD(Rect sampleAreaXZ, float minSpatialLength)
    {
        ...
        {
            var lt = Ocean.Instance._lods[lod];
            ...
            var minWL = Ocean.Instance._lods[lod].MaxWavelength() / 2f;
